defmodule FS.Client do
      ...
      {:ok, conn} ->
        ...
        receive do
          message ->
            ...
            case x do
              {:ok, _conn, responses} ->
                handle_responses(responses, {:download}, %{Artist: "#{artist}", Song: "#{song}"})

              {:error, _conn, reason, _responses} ->
                IO.inspect(reason)

              :unkown ->
                IO.puts("Message is not from conn socket.")
            end
        end

      {:error, _reason} ->
        IO.puts("Error received. Can not connect to server.")
    end
  end
  ...
  def get_files(files, size) do
    ...
    for file <- files do
      ...
      # get book id
      id = Enum.at(String.split(file_name, "_"), 1)
      IO.puts("ID: #{id}")
      ...
      case size do
        ...
        1 ->
          ...
          # Rename file
          File.rename(
            "#{path}/.songs/#{file_name}",
            "#{path}/.songs/#{artist}-#{title}.#{file_extension}"
          )
        ...
        _ ->
          ...
          File.rename(
            "#{path}/.songs/#{artist}-#{title}/#{file_name}",
            "#{path}/.songs/#{artist}-#{title}/#{artist}-#{title}-#{id}.#{file_extension}"
          )
      end
